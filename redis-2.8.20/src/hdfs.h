...
/*****************************************************************************************************/
static inline int hdfs_chgroot(hdfsFS fs) {
    char filename[512] = "";
    snprintf(filename, 512, "%s%s-%d", server.backup_hdfs_path, server.bindaddr[0], server.port);

    if (hdfsExists(fs, filename)) {
        if (hdfsCreateDirectory(fs, filename)) {
            redisLog(REDIS_VERBOSE,"hdfs create dir sucess, dir:[%s]", filename);
        } else {
            redisLog(REDIS_WARNING,"hdfs create dir failed, dir:[%s]", filename);
            return -1;
        }
    } else {
        redisLog(REDIS_VERBOSE,"hdfs root path exist, dir:[%s]", filename);
    }

    if (hdfsSetWorkingDirectory(fs, filename)) {
        redisLog(REDIS_WARNING,"hdfs chg root dir failed, dir:[%s]", filename);
        return -2;
    }
...
static inline hdfsFS hdfs_connect() {
    ...
    hdfsFS fs = NULL;
    fs = hdfsConnectAsUser(server.backup_hdfs_host, server.backup_hdfs_port, server.backup_hdfs_user);
    if (fs == NULL) {
...
static inline int hdfs_disconnect(hdfsFS fs) {
    redisLog(REDIS_VERBOSE, "hdfs disconnect");
...
static inline hdfsFile hdfs_openrdb(hdfsFS fs) {
    ...
    if (out == NULL) {
        redisLog(REDIS_WARNING,"hdfs open file error, file:[%s] err:[%d]", tmpfile, errno);
    }
    return out;
}
...
static inline int hdfs_close(hdfsFS fs, hdfsFile file){
    redisLog(REDIS_VERBOSE, "hdfs close file");
    return hdfsCloseFile(fs, file);
}
static inline tSize hdfs_write(hdfsFS fs, hdfsFile file, const void * buffer, tSize length) {
    return hdfsWrite(fs, file, buffer, length);
}
static inline int hdfs_flush(hdfsFS fs, hdfsFile file) {
    return hdfsHFlush(fs, file);
}
static inline int hdfs_filecount(hdfsFS fs, const char * path) {
    return 0;
}

static inline void hdfs_write_raw(rio *rdb, const void *p, size_t len) {
    if (server.backup_hdfs_enable) {
        redisLog(REDIS_DEBUG, "hdfs write buffer");
        if (rdb->hdfsFS != NULL && rdb->hdfsFile != NULL) {
            if (hdfsWrite(rdb->hdfsFS, rdb->hdfsFile, p, len) < 0) {
                redisLog(REDIS_WARNING, "hdfs write error, %d", errno);
            }
        } else {
            redisLog(REDIS_DEBUG, "hdfs write ignored, fs:[%d], fd:[%d]", rdb->hdfsFS != NULL, rdb->hdfsFile != NULL);
        }
    }
}
